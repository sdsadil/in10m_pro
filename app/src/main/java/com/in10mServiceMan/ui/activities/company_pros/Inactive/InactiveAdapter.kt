...
class InactiveAdapter(val mList: List<ActiveData>, val context: Context, val v: InactiveInterface): RecyclerView.Adapter<InactiveAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(p0: ViewHolder, p1: Int) {
        p0.itemview.acButton.text = "ACTIVATE"
        ...
        p0.itemview.clickLayout.setOnClickListener {
            ...
            v.inactiveTransaction(mList[p1].id)
        }

        p0.itemview.tv_status.setOnClickListener {
            SharedPreferencesHelper.putInt(context, Constants.SharedPrefs.User.COMMON_ID, mList[p1].id)
            v.inactiveEarningsTransaction()
        }
