...
public class MaskedEditText extends AppCompatEditText {
    ...
    public MaskedEditText(Context context, AttributeSet attr, String mask, char placeholder) {
        ...
        TypedArray a = context.obtainStyledAttributes(attr, com.github.reinaldoarrosi.maskededittext.R.styleable.MaskedEditText);
        final int N = a.getIndexCount();

        for (int i = 0; i < N; ++i) {
            int at = a.getIndex(i);

            if (at == com.github.reinaldoarrosi.maskededittext.R.styleable.MaskedEditText_mask) {
                mask = (mask.length() > 0 ? mask : a.getString(at));
            } else if (at == com.github.reinaldoarrosi.maskededittext.R.styleable.MaskedEditText_placeholder) {
